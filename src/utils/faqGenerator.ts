...
function isCapitalRoute(origin: string, destination: string): boolean {
  return CAPITAL_CITIES.has(origin.toLowerCase()) && 
         CAPITAL_CITIES.has(destination.toLowerCase());
}

// Generate FAQs for a route
export function generateFAQsForRoute(route: CollectionEntry<'routes'>): FAQ[] {
  const { origin, destination, originState, destinationState, distanceKm, transitDays } = route.data;
